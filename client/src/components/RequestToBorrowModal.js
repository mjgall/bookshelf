        ...
        <div>
          {friendsWithBook.length < 1 ? (
            <div className="text-center text my-1 text-gray-500 italic font-weight-light">
              <div>
                You don't have any friendsWithBook, create a household or
            ...
            <>
              <div className="text-lg font-weight-bold">Request From:</div>
              {friendsWithBook.map((connection, index) => {
                return (
                  <div
                    onClick={() =>
                      borrowFrom(connection.user_id, connection.full)
                    }
                    key={index}
