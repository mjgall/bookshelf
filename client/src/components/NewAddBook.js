  ...
  const Scan = () => {
    return (
      <div ref={scanTabRef} className="md:w-3/4 m-auto h-8">
        <Scanner currentTab={selection === "scan" ? true : false}></Scanner>
      </div>
    );
  };
      ...
      <Modal ref={addBookModal} header="Add book" type="addBook">
        <>
          {global.capturedBook ? (
            ...
            <>
              {selection ? (
                <div onClick={() => setSelection(undefined)} className="flex">
                  <ArrowLeftSquare
                ...
                <>
                  ...
                  <Button action={() => setSelection("scan")}>
                    Scan barcode
                  </Button>
                  <Button action={() => setSelection("manual")}>
                    Manually
                  </Button>
                </>
              )}
              {global.capturedBook ? (
                <BookDetails
                  closeModal={() => addBookModal.current.close()}
                ></BookDetails>
              ) : (
                <div>{renderActiveSelection()}</div>
              )}
            </>
          )}
        </>
