...
module.exports = (userId, bookId, page, count) => {
  return new Promise((resolve, reject) => {
    ...
    //         WHERE ( friendships.user_id_1 = ${userId} OR friendships.user_id_2 = ${userId} ) AND activities.hidden = FALSE AND global_books.id = ${bookId}
    // UNION
...
SELECT DISTINCT interacted_users.full AS interacted_user_name, activities.*, user_books.title, global_books.cover, users.id AS friend_id, users.email AS friend_email, users.full AS friend_full, users.picture AS friend_picture FROM friendships 
        ...
        JOIN global_books ON global_books.id = activities.object_id
        JOIN user_books ON global_books.id = user_books.global_id AND user_books.user_id = users.id AND global_books.id = ${bookId} AND user_books.private = FALSE
