...
module.exports = (userId) => {
    return new Promise((resolve, reject) => {
        const query = `SELECT DISTINCT IF(likes.liked_by, TRUE, FALSE) AS liked, activities.*, user_books.title, global_books.cover, users.id AS friend_id, users.email AS friend_email, users.full AS friend_full, users.picture AS friend_picture FROM friendships 
        JOIN users 
...
UNION
SELECT DISTINCT IF(likes.liked_by, TRUE, FALSE) AS liked, activities.*, global_books.title, global_books.cover, users.id AS friend_id, users.email AS friend_email, users.full AS friend_full, users.picture AS friend_picture FROM friendships 
        JOIN users
            ON ((friendships.user_id_1 = users.id OR friendships.user_id_2 = users.id) AND (friendships.accepted = TRUE AND friendships.declined != TRUE))
        JOIN activities ON users.id = activities.user_id 
        JOIN global_books ON global_books.id = activities.object_id
