...
module.exports = (app) => {
	app.get("/api/bootstrap", async (req, res) => {
		if (!req.user) {
			...
			res.send({
				currentUser,
				allBooks: userBooks
					.concat(householdBooks)
					.concat(borrowedBooks),
				books: {
					userBooks,
					householdBooks,
					borrowedBooks,
	...
	app.get("/api/bookclubs/:id", async (req, res) => {
		...
		bookClubMembers.forEach((member) => {
			if (member.book_club_id === bookClub.book_club_id) {
				bookClub = {
					...bookClub,
					members: [...bookClub.members, member],
				};
			}
		});
	...
	app.put("/api/bookclubs/:bookClubId/book", async (req, res) => {
		const response = await addBookClubsGlobalBooks(
	...
	app.get("/api/bookclubs", async (req, res) => {
		try {
			...
			const bookClubMembers = await getBookClubMembersByUserId(
				req.user.id
			);
	...
	app.post("/api/bookclubs/notes", async (req, res) => {
		...
		const response = await addNoteToClub(
			req.user.id,
			bookId,
			clubId,
			bookClubsGlobalBooksId,
			note
		);
		res.send(response);
	});
